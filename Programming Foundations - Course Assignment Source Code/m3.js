...
function storeValue() {
  ...
  if (result !== null) {
    alert("You entered: " + global);
    global = result;
  } else {
    alert("Current stored Data: " + global);
  }
  confirm(global);
}
...
function addTwoNumbers() {
  ...
  if (!isNaN(num1) || !isNaN(num2)) {
    ...
    alert("The final score is: " + Number(num1) + Number(num2));
  } else {
    alert("Wrong data");
  }
}
...
function extractMiddleString() {
  ...
  alert(
    text.slice(startIndex - length, startIndex + length + (text.length % 2))
  );
}
